...
$arduino/arduino-builder -dump-prefs -logger=machine -hardware "$arduino/hardware" -hardware "$HOME/.arduino15/packages" -tools "$arduino/tools-builder" -tools "$arduino/hardware/tools/avr" -tools "$HOME/.arduino15/packages" -built-in-libraries "$arduino/libraries" -libraries "$HOME/sketchbook/libraries" -fqbn=arduino:avr:uno -ide-version=10612 -build-path "/tmp/build" -warnings=none -prefs=build.warn_data_percentage=75 -verbose "$sketchbook/iLogger.ino"
$arduino/arduino-builder -compile -logger=machine -hardware "$arduino/hardware" -hardware "$HOME/.arduino15/packages" -tools "$arduino/tools-builder" -tools "$arduino/hardware/tools/avr" -tools "$HOME/.arduino15/packages" -built-in-libraries "$arduino/libraries" -libraries "$HOME/sketchbook/libraries" -fqbn=arduino:avr:uno -ide-version=10612 -build-path "/tmp/build" -warnings=none -prefs=build.warn_data_percentage=75 -verbose "$sketchbook/iLogger.ino"|tee /tmp/info.log
